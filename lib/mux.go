...
//  λλ mux.go  ▻20‹05‹07τ10›25›21›  ηη ▻20‹09‹26τ20›17›10› 
//  The dmcs lib package instantiates basic rest services to explore consul security properties using sidecarts and native connect  
//  Currently running a consul cluster made of 3 Arch Linux Servers 
    package lib
...
//  tls_get sends a key and gets a corresponding value via TLS
    func tls_get(source_svc, dest_svc, key string) (string, error) {
        ...
        cApiCli, err := api.NewClient(api.DefaultConfig())                // cApiCli is a pointer to the Consul API client
        if err != nil {
            fmt.Printf("ERROR: %s \n", err.Error())
        }

        thisSvc, err := connect.NewService(source_svc, cApiCli)           // thisSvc is a pointer to the Consul Connect agent service
        if err != nil {
            fmt.Printf("ERROR: %s \n", err.Error())
        }
        defer thisSvc.Close()
        ...
        dest_url := `https://` + dest_svc + `.service.dc1.consul/key/` + key
...
//  main http1.1 Integration Tests (ITest)
    ...
    func ITest1_rest_dmcs(msg string) {

        body, err := http_get(http_url1, msg)
        if err != nil {
            fmt.Printf("\nERROR: %v \nExiting ... \n", err.Error())
            os.Exit(1)
        }
    ...
    func ITest2_rest_dmcs(msg string) {

        body, err := http_get(http_url2, msg)
        if err != nil {
            fmt.Printf("\nERROR: %v \nExiting ... \n", err.Error())
            os.Exit(1)
        }
    ...
    func ITest3_rest_dmcs(msg string) {

        body, err := http_get(http_url3, msg)
        if err != nil {
            fmt.Printf("\nERROR: %v \nExiting ... \n", err.Error())
            os.Exit(1)
        }
    ...
    func ITest4_cConn_sidecar_rest_dmcs(msg string) {

        body, err := http_get(http_url4, msg)
        if err != nil {
            fmt.Printf("\nERROR: %v \nExiting ... \n", err.Error())
        }
        fmt.Printf("\nRESP:\n%v\n\n", body)
    ...
    func ITest5_cConn_sidecar_rest_dmcs(msg string) {

        body, err := http_get(http_url5, msg)
        if err != nil {
            fmt.Printf("\nERROR: %v \nExiting ... \n", err.Error())
        }
        fmt.Printf("\nRESP:\n%v\n\n", body)
    ...
    func ITest6_cConn_sidecar_rest_dmcs(msg string) {

        body, err := http_get(http_url6, msg)
        if err != nil {
            fmt.Printf("\nERROR: %v \nExiting ... \n", err.Error())
        }
        fmt.Printf("\nRESP:\n%v\n\n", body)
    ...
    func ITest7_cConn_native_rest_dmcs(msg string) {

        body, err := tls_get(`itest7native`, `svc7native`, msg)
        if err != nil {
    ...
    func ITest8_cConn_native_rest_dmcs(msg string) {

        body, err := tls_get(`itest8native`, `svc8native`, msg)
        if err != nil {
    ...
    func ITest9_cConn_native_rest_dmcs(msg string) {

        body, err := tls_get(`itest9native`, `svc9native`, msg)
        if err != nil {
...
//  main http servers
    ...
    func Svc7_cConn_native_rest_dmcs() {
        ...
        cApiCli, err := api.NewClient(api.DefaultConfig())            // instantiate a Consul API client
        if err != nil {
            fmt.Printf("ERROR: %s \n", err.Error())
        }

        thisSvc, err := connect.NewService("svc7native", cApiCli)     // instantiate a Consul-Connect HTTPS server
        if err != nil {
            fmt.Printf("ERROR: %s \n", err.Error())
        }
    ...
    func Svc8_cConn_native_rest_dmcs() {
        ...
        thisSvc, err := connect.NewService("svc8native", cApiCli)     // instantiate a Consul-Connect HTTPS server
        if err != nil {
            fmt.Printf("ERROR: %s \n", err.Error())
        }
        defer thisSvc.Close()                                       // clean up an close thisSvc before exiting this function
    ...
    func Svc9_cConn_native_rest_dmcs() {
        ...
        thisSvc, err := connect.NewService("svc9native", cApiCli)     // instantiate a Consul-Connect HTTPS server
        if err != nil {
            fmt.Printf("ERROR: %s \n", err.Error())
        }
        defer thisSvc.Close()                                       // clean up an close thisSvc before exiting this function
